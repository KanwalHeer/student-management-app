...
class Student {
    ...
    fees;
    paymentMethod;
    constructor(id, name, age, course, fees, paymentMethod) {
        this.id = id;
        this.name = name;
        this.age = age;
        this.course = course;
...
class StudentManagement {
    ...
    removeStudent(id) {
        ...
        if (this.students.length === initialLength) {
            console.log(chalk.yellow("Student not found with the provided ID. No student removed."));
        }
        else {
    ...
    displayStudents() {
        ...
        this.students.forEach((student) => {
            console.log(chalk.blue(`ID: ${student.id}`), chalk.green(`Name: ${student.name}`), chalk.magenta(`Age: ${student.age}`), chalk.yellow(`Course: ${student.course.name}`), chalk.red(`Fees: ${student.fees}`), chalk.green(`Payment Method: ${student.paymentMethod}`));
        });
    }
}
    ...
    while (true) {
        ...
        switch (choice) {
            case "Add Student":
                ...
                const newStudent = new Student(studentInfo.id, studentInfo.name, studentInfo.age, studentInfo.course, studentInfo.fees, studentInfo.paymentMethod);
    ...
    while (!isValidFees) {
        studentInfo = await inquirer.prompt([
            ...
            },
            {
                type: "number",
                name: "fees",
                message: (answers) => `Enter Fees (Course: ${answers.course.name}):`,
            },
            {
                type: "list",
                name: "paymentMethod",
        ...
        if (!isValidFees) {
            console.log(chalk.red("Warning: Entered fees are less than the specified fees for the course. Please enter the correct fees."));
        }
    }
    return studentInfo;
}
